...
void *receive_messages(void *socket, struct sockaddr_in *client_addr, socklen_t addr_len) {
    ...
    while (1) {
        if (recvfrom(*((int *)socket), message, MAX_LENGTH, 0, (struct sockaddr *)client_addr, &addr_len) > 0) {
...
void read_and_send_messages(int client_sockfd, struct sockaddr_in *client_addr, socklen_t addr_len) {
    ...
    while (1) {
        ...
        if (read(py_to_c, message, MAX_LENGTH) > 0) {
            ...
            sendto(client_sockfd, message, strlen(message), 0, (struct sockaddr *)client_addr, addr_len);
        }
    }
