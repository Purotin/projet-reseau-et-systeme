...
class Gui:
    ...
    def renderEditorButtons(self):
        ...
        # Clear bobs button
        self.smallButton(buttonX - (buttonWidth + 10), buttonY, buttonWidth, buttonHeight, "Clear bobs", self.guiSurface, lambda :( self.game.grid.removeAllBobs(Network.uuid_player), Network.sendRemoveAllBobs()), len(self.game.grid.getAllBobs()) == 0)

        # Clear food button
        self.smallButton(buttonX - (buttonWidth + 10), buttonY - (buttonHeight + 10), buttonWidth, buttonHeight, "Clear food", self.guiSurface, lambda : (self.game.grid.removeAllEdibles(Network.uuid_player), Network.sendRemoveAllFoods()), len(self.game.grid.getAllEdibleObjects()) == 0)
    ...
    def renderTooltip(self, objs, x, y, xTile, yTile):
        ...
        if isinstance(objs, Edible):
            ...
            # energy
            self.progressBar(10, 10, 280, 15, objs.energy / objs.energyMax, f"Energy: {int(objs.energy)} / {Settings.energyMax}", (194, 14, 14), tooltip)
